...
public class MyBST {
	...
	// It takes as input an array and build a BST tree from it
	...
	public int[] readIntoArray() {
		...
		List<Integer> res = asList(ints, node);
		
		return res.stream().mapToInt(Integer::intValue).toArray();
	}
	
	private List<Integer> asList(List<Integer> theList, Node node) {
		if (theList == null) {
			return theList;
		}
		if (node == null) {
			...
			asList(theList, node.left);
			theList.add(node.element);
			asList(theList, node.right);
		}
		return theList;
	}
